...
# newest_new_timestamp = datetime(1900, 1, 1)
# cachedfiles = []
# 
# 
# def process_changefile(changefile_type, file_sequence_number, new_timestamp):
...
#     global cachedfiles, newest_new_timestamp, global_tempfile_name
#     if new_timestamp != 0 and\
#        new_timestamp > newest_new_timestamp:
...
#         this_cachefile_name = this_cachefile_name + \
#                             "%09i.osc.gz" % file_sequence_number
#         this_cachefile_name = os.path.join(global_tempfiles,
#                                            this_cachefile_name)
#         if not os.path.exists(this_cachefile_name):
#             logging.info("%s changefile %i: downloading" %
#                          (changefile_type, file_sequence_number))
#             url = get_url(changefile_type) + "/"
#             url = url + ("%03i/%03i/%03i.osc.gz" % \
#                          (file_sequence_number / 1000000,
#                                             file_sequence_number / 1000 % 1000,
#                                                 file_sequence_number % 1000))
#             urllib.urlretrieve(url, this_cachefile_name)
#             logging.info("%s changefile %i: downloaded" %
#                      (changefile_type, file_sequence_number))
#         cachedfiles.append(this_cachefile_name)
# 
#     if len(cachedfiles) >= global_max_merge or \
#         (file_sequence_number == 0 and cachedfiles):
#     # at least one change files must be merged;
#     # merge all changefiles which are waiting in cache
#         logging.info("Merging changefiles.")
#         cmd = ["osmconvert", "--merge-versions"]
#         cmd.extend(global_osmconvert_arguments)
#         cmd.extend(cachedfiles)
#         cachedfiles = []
#         if os.path.exists(master_cachefile_name):
#             cmd.append(master_cachefile_name)
#         if newest_new_timestamp > datetime(1990, 1, 1):
#             cmd.append("--timestamp=" +\
#                        newest_new_timestamp.strftime("%Y-%m-%dT%H:%M:%SZ"))
#         cmd.append("--out-o5c")
#         sum_cache = open(master_cachefile_name_temp, 'wb')
#         result = subprocess.call(cmd, stdout=sum_cache, shell=False)
#         sum_cache.close()
#         if not os.path.exists(master_cachefile_name_temp) or \
#             getsize(master_cachefile_name_temp) < 10 or \
#             result != 0:  # "error" in result.lower():
#             raise AssertionError("Merging of changefiles failed: " + \
#                                  " ".join(cmd))
#         remove(master_cachefile_name)
#         os.rename(master_cachefile_name_temp, master_cachefile_name)
